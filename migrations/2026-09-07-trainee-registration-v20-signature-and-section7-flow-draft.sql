-- ============================================================
-- TRAINEE-REGISTRATION HTML — v20 DRAFT (two local refinements on the
-- currently PUBLISHED, user-approved layout-compaction version, aka v19)
-- ------------------------------------------------------------
-- The PUBLISHED version is immutable and is never read for its id/content
-- beyond a plain SELECT here, never updated, archived, or published by
-- this migration. This creates exactly ONE new DRAFT, at
-- COALESCE(MAX(version),0)+1 for the template, cloned from the CURRENT
-- PUBLISHED html_body/print_css with two targeted, local CSS/markup
-- changes only. It never touches merge_templates.current_published_version,
-- merge_jobs, merge_job_records, document_history, merge_template_fields,
-- or any candidate data. mapping_snapshot stays '[]' (same "resolve
-- current mapping" semantics every DRAFT gets).
--
-- REFINEMENT 1 — page 1 signature space (measured with real Chromium,
-- not guessed): page 1's organic content ended at 275.3mm of the 297mm
-- physical page, leaving ~21.7mm of unused space at the bottom.
-- .signature-space (used before both "Người làm đơn ký" and "Người tiếp
-- nhận") grows from 22px (~5.8mm) to 11mm — +~5.2mm each, +~10.4mm
-- combined, landing in the requested 8-12mm range. Page 1 organic content
-- after: 285.6mm — still ~11.4mm of headroom, no overflow to a new page.
--
-- REFINEMENT 2 — "7. QUY ĐỊNH KHI TẬP NGHỀ TẠI CÔNG TY" forced onto its
-- own physical page even though the preceding page (sections 1-6) had
-- ~75mm of unused space (measured: 221.5mm of 297mm). ROOT CAUSE (found
-- by direct inspection of the PUBLISHED html_body, not guessed): sections
-- 1-6 and section 7-10 were authored as TWO SEPARATE `.paper` divs — a
-- markup-level wrapper boundary, not a CSS break-before/break-after rule
-- targeting section 7 itself. Since `.paper` always carries
-- `page-break-after: always`, closing the first `.paper` unconditionally
-- forced a page break before section 7, regardless of how much space
-- remained. Fix: merge the two `.paper` divs into one continuous `.paper`
-- (remove the wrapper boundary and the now-incorrect
-- `style="margin-top:0"` override that assumed section 7 started a fresh
-- page), so Chromium's real print pagination decides the break naturally.
-- The section 7 heading also gets the `keep-with-next-small` utility
-- class (break-after:avoid / page-break-after:avoid) — already defined in
-- the shared renderer's LAYOUT_UTILITY_CSS, previously unused by any
-- template — so it can never be isolated alone at a page boundary; it is
-- guaranteed to stay with at least its first following item.
--
-- VERIFIED locally (real Chromium, worker's exact renderApplicantHtmlFromParts()
-- + page.pdf() config): section 7's heading now lands on page 2 together
-- with items a/b/c (previously alone atop page 3); the remainder flows
-- naturally onto page 3; total page count unchanged at 6. All 76
-- placeholders and all visible text are otherwise byte-identical to the
-- PUBLISHED source (only the .paper boundary, the one now-unneeded inline
-- margin-top:0, and the .signature-space height changed). Global
-- line-height, margins, typography, and every other page are untouched.
--
...
<div class="paper">
  ...
  <!-- ======================= TRANG 3 (merged into TRANG 2's .paper: real
       PDF pagination now decides the physical page break naturally, since
       section 1-6's compacted content no longer fills a full page on its
       own — see keep-with-next-small below for the section 7 heading) ======================= -->
  <div class="sec keep-with-next-small">7. QUY ĐỊNH KHI TẬP NGHỀ TẠI CÔNG TY:</div>
...
<div class="paper">
  ...
  $v20_css$/* ===== runtime parity (auto-generated): neutralise generic A4 renderer CSS; #doccss follows and wins ===== */
...
.trade-item {
  ...
  ps.retention_years, '[]'::jsonb, 'system-v20-signature-section7-flow', now(), now()
...
WHERE NOT EXISTS (
  ...
  WHERE existing.template_id = t.id
    AND existing.source_docx_name = 'trainee-registration/v20-signature-and-section7-flow-draft (page 1 signature space +~10mm; section 7 flows naturally after section 6 instead of forcing a new page)'
);
