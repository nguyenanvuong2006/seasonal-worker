-- ============================================================
-- PRODUCTION INCIDENT RECOVERY — FORWARD-ONLY, IDEMPOTENT
-- ============================================================
-- Incident (confirmed on Production main):
--   merge_templates.current_published_version = 6
--   merge_template_versions                   = EMPTY
--
-- Root cause: the destructive canonical cleanup migration
--   migrations/2026-08-24-trainee-registration-canonical-cleanup.sql
-- was rerun by the recurring Production migration runner. Its
--   DELETE ... status != 'DRAFT'
-- removed the canonical PUBLISHED v6, and because the v7 DRAFT
-- migration was missing from the runner, no replacement existed.
--
-- This recovery migration repairs ONLY that incident:
--   A. Targets ONLY template a1b2c3d4-e5f6-7890-abcd-ef1234567890
--      (the canonical trainee-registration template family).
--   B. Detects a dangling current_published_version (pointing at a
--      version row that no longer exists).
--   C. Creates EXACTLY ONE v7 DRAFT from the approved operator body
--      (byte-identical to
--      migrations/2026-08-24-trainee-registration-v7-operator-test2-draft.sql,
--      body SHA-256 7cb43551d3d4f5178ce203a176a7004aa7e3994ecad2276ef593b6fe401116c1).
--      The inserted canonical version is DETERMINISTICALLY version 7
--      (never MAX(version)+1), and the insert is skipped if a row with
--      the same source_docx_name or the same body SHA-256 already exists.
--   D. Does NOT publish: status stays 'DRAFT', published_at untouched,
--      current_published_version never set to a value here.
--   E. Sets current_published_version to NULL ONLY when it points at a
--      missing version — runtime then fails closed (no silent fallback)
--      until an operator explicitly publishes v7.
--   F. Deletes NOTHING: no DELETE/TRUNCATE/DROP anywhere. Does not touch
--      merge_templates rows other than the dangling-pointer fix above,
--      and never touches merge_template_fields, merge_jobs,
--      merge_job_records, document_history, or archive_runs.
--   G. Idempotent: running it twice (or N times) never creates v8/v9 and
--      never duplicates v7 — guarded by source_docx_name match, body
--      SHA-256 match, and the UNIQUE(template_id, version) backstop.
--
-- Source provenance (from the approved v7 migration):
...
-- digest() for the SHA-256 content guard below (same extension schema.sql creates).
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- ----------------------------------------------------------------
-- STEP 1 — recreate the v7 DRAFT, deterministically version 7.
-- Insert happens ONLY when ALL of these hold:
--   * no existing row for this template already carries the v7
--     source_docx_name (same identity key the v7 migration uses);
--   * no existing row for this template already carries the v7 body
--     (SHA-256 match — catches a body re-saved under a different name);
--   * version 7 is still free for this template.
-- Otherwise this statement inserts nothing (safe re-run).
-- ----------------------------------------------------------------
...
WHERE t.id = 'a1b2c3d4-e5f6-7890-abcd-ef1234567890'
  AND NOT EXISTS (
    SELECT 1
    FROM merge_template_versions e
    WHERE e.template_id = t.id
      AND (
        e.source_docx_name = 'trainee-registration/test(2).html (operator-provided canonical HTML; preview UI stripped) v7'
        OR encode(digest(coalesce(e.html_body, ''), 'sha256'), 'hex') =
...
-- ----------------------------------------------------------------
-- STEP 2 — fail closed on a dangling current_published_version.
-- NULL it ONLY when the referenced version row does not exist for
-- this template. A pointer to an existing row (any status) is left
-- untouched; NULL (already fail-closed) stays NULL.
-- ----------------------------------------------------------------
UPDATE merge_templates t
SET current_published_version = NULL,
    updated_at = now()
WHERE t.id = 'a1b2c3d4-e5f6-7890-abcd-ef1234567890'
  AND t.current_published_version IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM merge_template_versions v
    WHERE v.template_id = t.id
      AND v.version = t.current_published_version
  );

COMMIT;

-- ----------------------------------------------------------------
-- Read-only verification (no writes): after this migration,
-- current_published_version must be NULL (fail-closed) and exactly
-- one v7 DRAFT must exist for the canonical template.
-- ----------------------------------------------------------------
