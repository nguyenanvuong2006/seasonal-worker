-- ============================================================
-- TRAINEE-REGISTRATION HTML — layout-compaction DRAFT (evolution of v18)
-- ------------------------------------------------------------
-- PUBLISHED v18 is immutable and is never read, updated, archived, or
-- published here. This creates exactly ONE new DRAFT, at
-- COALESCE(MAX(version),0)+1 for the template, from v18's own
-- (verified, sha256-checked) html_body/print_css. It never touches
-- merge_templates.current_published_version, merge_jobs,
-- merge_job_records, document_history, merge_template_fields, or any
-- candidate data. mapping_snapshot stays '[]' — same "resolve current
-- mapping" semantics every DRAFT gets; publishTemplateVersion() is the
-- sole authority that freezes the CURRENT non-orphaned
-- merge_template_fields into mapping_snapshot at PUBLISH time.
--
-- ROOT CAUSE (measured locally with real Chromium against v18's own
-- verified content, not guessed): v18's dense, government-form spacing
-- (line-height 1.42, generous paragraph/section margins, 24-26mm
-- signature spacer divs) pushed 4 of its 6 logical .paper sections
-- (each already intentionally one physical page, per the source's own
-- "TRANG 1".."TRANG 6" authoring comments) past the 297mm physical page
-- boundary, inflating real Chromium PDF output from the intended 6
-- pages to 10 — with isolated candidate-name/signature fragments
-- landing alone on near-empty trailing pages (TRANG 3's and TRANG 6's
-- sign-block) and a stray attach-box row spilling onto its own page
-- (TRANG 1). Note: `.paper`'s own `min-height:297mm` is NOT the cause
-- of the extra pages — html-renderer.ts's pageGeometryCss() (shared
-- renderer code, deliberately NOT touched by this migration) always
-- re-applies min-height:297mm last in the cascade regardless of what a
-- template's own CSS declares, by design (see its docblock). The fix
-- below only tightens vertical density enough that every section's
-- REAL content again fits within one physical page, matching the
-- source's own original per-section-one-page intent.
--
-- CHANGES vs v18 (verified byte-diff — nothing else touched):
--   print_css:
--     - line-height 1.42 -> 1.22 (still fully legible at 10.5pt)
--     - .sec / .line / .tight / .hang / .mt-4 / .mt-8 / .doc-header
--       margins tightened (roughly -30 to -60%)
--     - .attach-box / .attach-box td padding+margin tightened
--     - .signature-space height 48px -> 22px
--   html_body:
--     - the 4 inline sign-block spacer divs (24mm/26mm, purely blank
--       signing room, no text) reduced to 16mm — still a real, usable
--       signature area
--   margin_top_mm/margin_bottom_mm/margin_left_mm/margin_right_mm: left
--   at the column default (10/10/12/12) — not needed once the above
--   density fixes were applied; verified sufficient with real Chromium
--   against both this fixture's data AND a stress test using much
--   longer realistic field values (long name/address/bank/employer
--   strings) — still exactly 6 pages, still no isolated signature.
--
-- Every placeholder/mapping/visible text is byte-for-byte identical to
-- v18 (verified: 76/76 placeholders match, and the body is identical
-- once only those 4 spacer heights are normalized). Nothing here can
-- change which fields a real merge fills in or how a real candidate's
-- data resolves.
--
-- VERIFIED locally (real Chromium, worker's exact renderApplicantHtmlFromParts
-- + page.pdf() config, both this fixture's data and a long-value stress
-- test): 10 pages -> 6 pages, matching the source's intended 6 sections;
-- no isolated candidate name/signature; no clipping; no lost content.
--
-- Idempotent: re-running finds the DRAFT it already created (matched by
-- source_docx_name) and skips re-inserting it.
-- ============================================================
...
published_source AS (
  SELECT v.retention_years
  FROM merge_template_versions v
  JOIN target_template t ON v.template_id = t.id AND v.version = t.current_published_version
  WHERE v.status = 'PUBLISHED'
),
next_version AS (
  SELECT COALESCE(MAX(v.version), 0) + 1 AS version
  FROM merge_template_versions v
  JOIN target_template t ON v.template_id = t.id
)
...
<div class="tight work-options">
  <span>Đã có tiền án, tiền sự trước đây:</span>
  <span>{{Tien_an_tien_su_Khong}} Không</span>
  <span>{{Tien_an_tien_su_Co}} Có</span>
</div>

<div class="tight work-options">
  <span>Đã từng tập nghề/ làm việc cho Cty Dalat Hasfarm:</span>
  <span>{{Da_tung_lam_DHF_Khong}} Không</span>
  <span>{{Da_tung_lam_DHF_Co}} Có</span>
</div>

<div class="tight">Vị trí tập nghề/ làm việc trước đây:</div>

<div class="tight pl-1 work-options">
  <span>{{Loai_cong_viec_Nhan_vien}} Nhân viên</span>
  <span>{{Loai_cong_viec_Cong_nhan}} Công nhân</span>
  <span>{{Loai_cong_viec_Lao_dong_tap_nghe}} Lao động tập nghề</span>
</div>

<div class="tight pl-1 i work-options">
  <span>Khu vực làm việc:</span>
  <span>{{Khu_vuc_Da_Lat}} Đà Lạt</span>
  <span>{{Khu_vuc_Da_Quy}} Đa Quý</span>
  <span>{{Khu_vuc_Da_Ron}} Đạ Ròn</span>
  <span>{{Khu_vuc_Lam_Ha}} Lâm Hà</span>
  <span>{{Khu_vuc_Khac}} Khu vực khác</span>
</div>

<div class="tight">
  Công việc hiện tại: {{Cong_viec_hien_tai_Sinh_vien}} Sinh viên trường {{Ten_truong}}
</div>
  <div class="tight">{{Cong_viec_hien_tai_Khac}} Khác {{Cong_viec_hien_tai_khac}}</div>
  

<div class="tight work-options">
  <span>Tài khoản ngân hàng (chính chủ):</span>
  <span>{{TKNH_Da_co}} Đã có</span>
  <span>{{TKNH_Chua_co}} Chưa có</span>
</div>
...
<div class="tight b trade-options">
  <span class="trade-item">{{Tap_nghe_Trong_cham_soc_thu_hoach}} Trồng, chăm sóc, thu hoạch</span>
  <span class="trade-item">{{Tap_nghe_Ban_hang}} Bán hàng</span>
  <span class="trade-item">{{Tap_nghe_Dong_goi}} Đóng gói</span>
  <span class="trade-item">{{Tap_nghe_Khac}} Khác {{Cong_viec_khac}}</span>
</div>

<div class="tight">
  Thời gian đăng ký tập nghề: từ ngày {{Ngay_nhan_viec}}
</div>

  <div class="line mt-4 b">
    III/ <span class="u">CAM KẾT CỦA NGƯỜI LÀM ĐƠN</span>
  </div>

  <div class="line justify">
    Tôi xin cam kết sẽ chấp hành tốt mọi nội quy, chính sách và quy định Tập Nghề cụ thể của Công ty trong suốt thời gian tập nghề.
    Nếu tôi có bất cứ hành vi sai phạm nào làm ảnh hưởng đến tài sản, uy tín, hoặc trật tự kỷ luật chung của Công ty,
    Công ty có quyền chấm dứt ngay thời gian tập nghề với tôi và xử lý sai phạm,
    yêu cầu tôi bồi thường thiệt hại (nếu có).
  </div>

  <div class="line justify">
    Mọi thông tin tôi cung cấp bên trên là đúng sự thật,
    nếu có sai sót tôi xin hoàn toàn chịu trách nhiệm theo quy định của pháp luật hiện hành.
  </div>

  <div class="line justify">
    Giấy đăng ký này thay cho thỏa thuận tập nghề giữa tôi và Công ty.
  </div>

<!-- KHOẢNG TRỐNG ĐỂ NGƯỜI LÀM ĐƠN KÝ -->
<div class="signature-space"></div>

<div class="line b equal-columns-2">
  <span>Người làm đơn ký: {{Ho_ten}}</span>
  <span>Ngày đăng ký: {{Ngay_nhan_viec}}</span>
</div>



  <div class="line mt-4 b u">
    GHI NHẬN CỦA PHÒNG NHÂN SỰ:
  </div>
...
<div class="line b equal-columns-2">
  <span>Người tiếp nhận: {{Nguoi_tiep_nhan}}</span>
  <span>Ngày: {{Ngay_tiep_nhan}}</span>
</div>

  <!-- ======================= ĐÍNH KÈM — VẪN THUỘC TRANG 1 ======================= -->

  <div class="attach-box mt-4">
    <div class="b u">Đính kèm</div>

    <table>
      <tr>
        <td class="c1">- Cam kết về thuế TNCN</td>
        <td class="c2">Có</td>
        <td class="c3">Chưa</td>
      </tr>
...
<div class="paper">
  ...
  $v19_css$/* ===== runtime parity (auto-generated): neutralise generic A4 renderer CSS; #doccss follows and wins ===== */
...
.paper{
  ...
  font-size:10.5pt; line-height:1.22; color:#000;
  width:210mm; padding:12mm 14mm;
  position:relative; page-break-after:always;
}
.paper:last-child{page-break-after:auto}
.doc-header{display:table;width:100%;table-layout:fixed;margin-bottom:4mm}
...
.sec{font-weight:bold;text-decoration:underline;margin:2.5mm 0 1.3mm}
.line{margin:0 0 1.5mm} .tight{margin:0 0 .3mm}
.indent{text-indent:10mm}
.pl-1{padding-left:10mm} .pl-2{padding-left:18mm}
.mt-4{margin-top:2.5mm} .mt-8{margin-top:5mm}
.hang{padding-left:10mm;text-indent:10mm;text-align:justify;margin:0 0 1.5mm}
...
.attach-box{border:1px solid #000;padding:1.5mm 3mm;font-size:9.5pt;font-style:italic;margin:2.5mm 0 0}
.attach-box table{width:100%;border-collapse:collapse}
.attach-box td{padding:.2mm 0}
.attach-box td.c1{width:45%} .attach-box td.c2{width:15%} .attach-box td.c3{width:40%}
.quochieu{text-align:center;font-weight:bold;font-size:11pt;line-height:1.3}
...
.sign-3{width:100%;border-collapse:collapse;margin-top:3mm}
...
.trade-item {
  ...
  ps.retention_years, '[]'::jsonb, 'system-layout-compaction', now(), now()
FROM target_template t
JOIN next_version nv ON true
JOIN published_source ps ON true
WHERE NOT EXISTS (
  SELECT 1 FROM merge_template_versions existing
  WHERE existing.template_id = t.id
    AND existing.source_docx_name = 'trainee-registration/layout-compaction-draft (evolution of v18: tightened spacing so all 6 sections fit their own intended single physical page — 10 real Chromium pages -> 6)'
);

-- Read-only verification only. No publish, no template pointer update, no jobs.
SELECT version, status, mapping_snapshot, margin_top_mm, margin_bottom_mm, margin_left_mm, margin_right_mm
FROM merge_template_versions
WHERE source_docx_name = 'trainee-registration/layout-compaction-draft (evolution of v18: tightened spacing so all 6 sections fit their own intended single physical page — 10 real Chromium pages -> 6)';
