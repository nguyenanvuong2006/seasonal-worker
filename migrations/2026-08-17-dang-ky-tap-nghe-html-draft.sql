...
-- ------------------------------------------------------------
-- Phase 5: chuyển template Dang_ky_Tap_nghe (Google Doc gốc) sang
-- HTML/CSS print template. Version này là bản TÁI DỰNG LAYOUT từ
-- template.ts (registry) — CHƯA được visual verification → chỉ tạo
-- DRAFT, KHÔNG publish. Admin phải Preview/so sánh với reference
-- trước khi bấm Publish (spec D + H).
--
-- NGUYÊN TẮC AN TOÀN: idempotent, non-destructive.
-- ============================================================
...
WHERE t.google_doc_id = '10D0tG71CbllIZe7DaosYNW3vK7QnP76Yq4UC9FMEiUE'
  AND NOT EXISTS (
    SELECT 1 FROM merge_template_versions v
    WHERE v.template_id = t.id AND v.version = 1
  );

-- Đánh dấu template có bản HTML (vẫn chưa publish — batch giữ GOOGLE_DOCS).
...
-- ============================================================
-- Xong. Batch PDF vẫn dùng GOOGLE_DOCS tới khi visual verification đạt.
-- ============================================================
