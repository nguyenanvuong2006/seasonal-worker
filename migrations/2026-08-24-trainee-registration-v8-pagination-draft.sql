-- ============================================================
-- TRAINEE-REGISTRATION HTML — v8 DRAFT (pagination-only follow-up to v7)
-- v7 is immutable and is never read, updated, archived, or published here.
-- This creates ONLY version 8 as DRAFT. It preserves all 49 placeholders and
-- mapping semantics; mapping_snapshot remains [] until an operator publishes.
-- The only document change is a structural .paper boundary immediately before
-- the DALAT HASFARM / QUY ĐỊNH VỀ TẬP NGHỀ header, plus an anti-orphan rule
-- scoped exclusively to the regulations signature container.
-- Body SHA-256: d68b329629c7b5ac7722f207035e057a6a757c44966c613a03852c3bbadf794e
-- Print CSS SHA-256: 55e91fdc2f5b3d690ceba6bc29c8de4147350b661035868a8677c9479a02c757
-- ============================================================
...
$v8_css$,
  'trainee-registration/test(2).html (operator-provided canonical HTML; v8 explicit regulations pagination only) v8', 3, '[]'::jsonb, 'system', now(), now()
FROM target_template t
WHERE NOT EXISTS (
  SELECT 1 FROM merge_template_versions existing
  WHERE existing.template_id = t.id
    AND (existing.version = 8 OR existing.source_docx_name = 'trainee-registration/test(2).html (operator-provided canonical HTML; v8 explicit regulations pagination only) v8')
);
...
WHERE source_docx_name = 'trainee-registration/test(2).html (operator-provided canonical HTML; v8 explicit regulations pagination only) v8';
