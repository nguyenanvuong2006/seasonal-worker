/**
 * LAYOUT CAPABILITY UTILITIES (v13) — five opt-in, class-scoped CSS
 * capabilities: `.equal-columns-2`, `.keep-with-next-small`, `.variable-length`,
 * `.manual-page-break`, `.keep-together`. They are additive to A4_PRINT_CSS;
 * no existing template references them, so their presence must be a no-op
 * for every already-PUBLISHED template.
 *
 * `.manual-page-break` and `.keep-together` are the standard reusable
 * pagination-authoring utilities exposed by Template Editor's "Công cụ phân
 * trang" tools (Chèn ngắt trang / Giữ cùng trang) — one shared definition
 * used by every template instead of per-template CSS duplication.
 */
...
test("LAYOUT_UTILITY_CSS exposes all five scoped layout utilities", () => {
  ...
  assert.match(LAYOUT_UTILITY_CSS, /\.manual-page-break\s*\{[^}]*page-break-before:\s*always/);
  assert.match(LAYOUT_UTILITY_CSS, /\.keep-together\s*\{[^}]*break-inside:\s*avoid/);
  assert.match(LAYOUT_UTILITY_CSS, /\.keep-together\s*\{[^}]*page-break-inside:\s*avoid/);
});
...
test("utilities are class-scoped (never broad tag selectors) so the layout scanner cannot flag them", () => {
  ...
  // / `page-break-inside` selector would raise GLOBAL_BREAK_INSIDE_AVOID.
  // `.keep-with-next-small` uses break-after (a different property) and
  // `.keep-together` uses break-inside but is class-scoped (isBroadSelector
  // returns false whenever a selector chunk carries a class or id) — neither
  // is flagged.
...
test("wrapHtmlDocument always injects the utilities alongside the base print CSS", () => {
  ...
  assert.match(html, /\.manual-page-break/);
  assert.match(html, /\.keep-together/);
});
