/**
 * Nhập mã công nhật page — GLOBAL DATE RANGE STANDARDIZATION DOM test.
 * Real render in jsdom (same render-tsx.ts harness as other page.test.tsx
 * files). Proves the screen shows "Từ ngày" / "Đến ngày" (a real date
 * RANGE), never a single "Ngày" label, and the shared DateRangeFilter
 * renders with mobile-first responsive classes.
 */
...
const ROWS = [
  { dailyApplicationId: "app-1", cccd: "010000000001", fullName: "Nguyen Van A", deptId: "d1", deptName: "Đóng gói", groupName: null, startingDate: null, dwImportedAt: new Date().toISOString(), dwDataId: "dw-1", code: "CN-001", dailyCodeUpdatedAt: null, dailyCodeUpdatedBy: null },
];
...
async function renderPage(env: RenderEnv) {
  (globalThis as Record<string, unknown>).fetch = async (input: unknown) => {
    ...
    if (url.includes("/api/administration/daily-code")) return { ok: true, status: 200, json: async () => ({ rows: ROWS }) } as unknown as Response;
