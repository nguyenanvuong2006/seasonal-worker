...
export default function SystemHealthPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="animate-slide-up overflow-hidden rounded-[24px] border border-black/5 bg-[#0b4527] p-6 text-white shadow-[0_18px_45px_rgba(18,36,25,0.14)] sm:p-8">
        <div className="flex items-center gap-4">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#155c38] ring-1 ring-white/10">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-white/72">Database, Import Engine, Audit Log, đồng bộ dữ liệu — tất cả trong 1 nơi.</p>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {QUICK_ACTIONS.map((a) => (
            <Link
              key={a.href}
              href={a.href}
              className="group flex flex-col gap-2 rounded-2xl border border-white/10 bg-[#124f31] p-4 transition-colors hover:bg-[#17613c]"
            >
              <a.icon className="h-5 w-5 text-gold-300 transition group-hover:scale-105" />
              <span className="text-sm font-black">{a.label}</span>
              <span className="text-[11px] text-white/62">{a.desc}</span>
            </Link>
          ))}
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            {[
              ...
              <div key={k.l} className="rounded-[18px] border border-black/5 bg-[#0F3D23] p-4 text-white shadow-[0_8px_22px_rgba(18,36,25,0.08)]">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader title={<span className="flex items-center gap-2"><Database className="h-4 w-4" /> Database (Neon)</span>} />
              <CardContent className="space-y-2 text-sm">
                <p>Dung lượng đang dùng: <b>{stats.dbSize.size_pretty}</b></p>
                <p><b>{stats.tablesAndIndexes.tables}</b> bảng · <b>{stats.tablesAndIndexes.indexes}</b> index</p>
                <a href="https://console.neon.tech" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary hover:underline">
            ...
            <Card>
              <CardHeader title={<span className="flex items-center gap-2"><GitBranch className="h-4 w-4" /> Phiên bản cấu hình</span>} />
              <CardContent className="space-y-1.5 text-sm">
                <p>Workflow: <b>{stats.workflowVersion.stages}</b> bước · cập nhật gần nhất <b>{fmt(stats.workflowVersion.last_updated)}</b></p>
                <p>Metadata: <b>{stats.metadataVersion.fields}</b> trường · cập nhật gần nhất <b>{fmt(stats.metadataVersion.last_updated)}</b></p>
              </CardContent>
            </Card>

            <Card>
              <CardHeader title={<span className="flex items-center gap-2"><Bell className="h-4 w-4" /> Hàng đợi thông báo</span>} />
              <CardContent className="space-y-1.5 text-sm">
                <p><Badge tone="amber">{stats.notificationQueue.queued} đang chờ</Badge>{" "}<Badge tone="green">{stats.notificationQueue.sent} đã gửi</Badge>{" "}{stats.notificationQueue.failed > 0 && <Badge tone="red">{stats.notificationQueue.failed} lỗi</Badge>}</p>
                <a href="/admin/notifications" className="inline-block text-primary hover:underline">Xem hàng đợi →</a>
              </CardContent>
            </Card>

            <Card>
              <CardHeader title={<span className="flex items-center gap-2"><CalendarClock className="h-4 w-4" /> Scheduler</span>} />
              <CardContent className="space-y-1.5 text-sm">
                {stats.scheduler.map((j) => (
                  <p key={j.job_key}><Badge tone={j.is_active ? "green" : "gray"}>{j.label}</Badge> — lần chạy gần nhất <b>{fmt(j.last_run_at)}</b> {j.last_status ? `(${j.last_status})` : ""}</p>
                ))}
              </CardContent>
            </Card>

            <Card>
              <CardHeader title={<span className="flex items-center gap-2"><GitCommit className="h-4 w-4" /> Build hiện tại (Vercel)</span>} />
              <CardContent className="space-y-1 text-sm">
                <p>Commit: <b className="font-mono">{stats.build.commit ?? "—"}</b>{stats.build.branch ? ` (${stats.build.branch})` : ""}</p>
                <p>Môi trường: <b>{stats.build.env}</b> {stats.build.region ? `· Vùng: ${stats.build.region}` : ""}</p>
                <a href="https://vercel.com/dashboard" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary hover:underline">Mở Vercel Dashboard <ExternalLink className="h-3 w-3" /></a>
              </CardContent>
            </Card>

            <Card>
              <CardHeader title={<span className="flex items-center gap-2"><Clock className="h-4 w-4" /> Hoạt động gần nhất</span>} />
              <CardContent className="space-y-1.5 text-sm">
