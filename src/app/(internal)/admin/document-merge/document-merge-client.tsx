...
function DocumentMergeContent({ permissions, role }: { permissions: string[]; role: string }) {
  const permSet = useMemo(() => new Set(permissions), [permissions]);
  const visibleTabs = useMemo(() => TABS.filter((t) => canSeeTab(t.id, permSet, role)), [permSet, role]);
  const defaultTab = useMemo(() => firstPermittedTab(permSet, role), [permSet, role]);

  const searchParams = useSearchParams();
  const templateParam = searchParams.get("template") || "";
  const requestedTab = searchParams.get("tab") as TabType | null;
  const initialTab: TabType | null = templateParam && canSeeTab("merge", permSet, role) ? "merge" : requestedTab && canSeeTab(requestedTab, permSet, role) ? requestedTab : defaultTab;
  ...
  useEffect(() => {
    if (templateParam && canSeeTab("merge", permSet, role)) {
  ...
  const selectForMerge = (templateId: string) => {
    if (!canSeeTab("merge", permSet, role)) return;
    setSelectedTemplateId(templateId);
    setActiveTab("merge");
  };

  if (!activeTab) {
    // Module nav đã yêu cầu >= 1 quyền document_merge để hiện — nếu tới đây mà
    // vẫn không có tab nào khả dụng thì quyền vừa bị thu hồi giữa chừng phiên
    // làm việc (không phải lỗi hệ thống) — báo rõ, không hiện trang trắng/403 mơ hồ.
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 text-center text-sm text-slate-500">
        Bạn không còn quyền truy cập tính năng nào trong Document Merge. Liên hệ quản trị viên nếu đây là nhầm lẫn.
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex gap-2">
              {canSeeTab("templates", permSet, role) && (
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        {activeTab === "verification" && canSeeTab("verification", permSet, role) && <VerificationPanel />}
      </main>
    </div>
  );
}

export default function DocumentMergeClient({ permissions, role }: { permissions: string[]; role: string }) {
  return (
    <Suspense fallback={<div className="flex min-h-screen items-center justify-center bg-slate-50 text-sm text-slate-500">Đang tải Document Merge Center...</div>}>
      <DocumentMergeContent permissions={permissions} role={role} />
    </Suspense>
  );
}
...
function HistoryTab({ canDelete }: { canDelete: boolean }) {
  ...
  const remove = async (jobId: string) => {
    // Frontend visibility is not authorization — backend vẫn tự kiểm tra
    // document_merge.history.delete độc lập, nút này chỉ ẩn/khoá để tránh
    // gọi API rồi nhận 403 vô ích. Chặn ở đây chỉ là UX, không phải bảo mật.
    if (!canDelete) return;
