...
test("status hợp lệ trên query string được truyền đúng xuống getMealEligibleWorkers", async () => {
  const { mod, calls, audits } = loadRoute({ scope: null });
  const GET = mod.GET as (req: Request) => Promise<{ status: number }>;
  const res = await GET(makeReq("http://localhost/api/meal/export?date=2026-08-17&status=INELIGIBLE"));

  assert.equal(res.status, 200);
  assert.equal(calls[0].filters.status, "INELIGIBLE");
  assert.equal(audits[0].detail.status, "INELIGIBLE");
});

test("status mặc định là ELIGIBLE khi không truyền trên query string (giữ hành vi cũ)", async () => {
  const { mod, calls } = loadRoute({ scope: null });
  const GET = mod.GET as (req: Request) => Promise<{ status: number }>;
  await GET(makeReq("http://localhost/api/meal/export?date=2026-08-17"));

  assert.equal(calls[0].filters.status, "ELIGIBLE");
});
