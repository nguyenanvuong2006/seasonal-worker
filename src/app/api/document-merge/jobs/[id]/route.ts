...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      // Output URLs (batch outputs temporary — TTL theo batchExpiresAt).
      outputPdfUrl: job.outputPdfUrl,
      outputZipUrl: job.outputZipUrl,
      batchExpiresAt: job.batchExpiresAt,
      // Cho UI: technical details chỉ hiển thị cho Admin.
      viewerRole: guard.session.role,
      items,
    });
  } catch (error) {
