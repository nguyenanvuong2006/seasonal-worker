...
import { computeFileChecksum, createWorkforceMasterBatch, mergeWorkforceMasterChunk } from "@/lib/data-management/import-workforce-master";
...
/**
 * Bounded-chunk, resumable import (mission section 38) — never one giant
 * transaction for the whole file. First call sends `file` (+ optional
 * datasetMode) and creates the batch; every call (including the first)
 * processes exactly one chunk (≤300 rows) and returns `{done}` — the client
 * loops, sending `{batchId}` alone on subsequent calls, until done=true.
 */
export async function POST(req: Request) {
  ...
  if (existingBatchId) {
    ...
    const created = await createWorkforceMasterBatch({
      fileName: file.name,
      checksum,
      rows,
    ...
    await writeAudit(guard.session, "DATA_MANAGEMENT_IMPORT_STARTED", "workforce_data_import_batches", { batchId, importType: "WORKFORCE_MASTER", fileName: file.name, totalRows: rows.length, datasetMode }, "IMPORT");
  }

  const chunk = await mergeWorkforceMasterChunk(batchId);
  if (chunk.done) {
    await writeAudit(guard.session, "DATA_MANAGEMENT_IMPORT_COMPLETED", "workforce_data_import_batches", { batchId, importType: "WORKFORCE_MASTER" }, "IMPORT");
  }
