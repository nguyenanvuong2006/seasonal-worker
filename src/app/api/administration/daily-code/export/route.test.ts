...
/* ============================================================
   KIỂM THỬ TẦNG ROUTE — GET /api/administration/daily-code/export
   ------------------------------------------------------------
   Theo đúng mẫu src/app/api/meal/export/route.test.ts:
     • deptId ngoài Data Scope -> 403, KHÔNG gọi getDailyCodeRows,
       KHÔNG xuất file.
     • deptId + q + status hợp lệ được truyền ĐÚNG xuống
       getDailyCodeRows — ĐÚNG hàm dùng chung với GET
       /api/administration/daily-code (list) — file xuất luôn khớp
...
test("deptId ngoài Data Scope -> 403, KHÔNG gọi getDailyCodeRows, KHÔNG xuất file", async () => {
  const { mod, calls } = loadRoute({ scope: ["dept-A"] });
  const GET = mod.GET as (req: Request) => Promise<{ status: number; body: Record<string, unknown> }>;
  const res = await GET(makeReq("http://localhost/api/administration/daily-code/export?date=2026-08-17&deptId=dept-OUTSIDE"));

  assert.equal(res.status, 403);
  assert.equal(calls.length, 0, "fail-closed: không được truy vấn/xuất dữ liệu ngoài Data Scope");
});

test("deptId + q + status hợp lệ được truyền đúng xuống getDailyCodeRows — CÙNG hàm với GET /api/administration/daily-code", async () => {
  ...
  const res = await GET(makeReq("http://localhost/api/administration/daily-code/export?date=2026-08-17&deptId=dept-A&q=Nguyen&status=DONE"));
  ...
  assert.equal(audits[0].detail.status, "DONE");
});

test("status mặc định là ALL khi không truyền trên query string", async () => {
  const { mod, calls } = loadRoute({ scope: null });
  const GET = mod.GET as (req: Request) => Promise<{ status: number }>;
  await GET(makeReq("http://localhost/api/administration/daily-code/export?date=2026-08-17"));
