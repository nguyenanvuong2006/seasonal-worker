...
import { buildIndividualPdfFilename, buildStorageKey } from "@/lib/document-merge/filename.ts";
...
async function processItem(item: QueueItem, jobCtx: JobContext): Promise<void> {
  ...
  const cccd = String(recordData.cccd ?? "");
  const filename = buildIndividualPdfFilename(item.sortOrder, fullName, cccd);
  const storageKey = buildStorageKey(jobCtx.jobId, "individual", filename);

  const storage = getStorageProvider();
  const stored = await storage.put(storageKey, bytes, "application/pdf");

  await completeItem(item.id, { pdfUrl: stored.url, storageKey });
}
