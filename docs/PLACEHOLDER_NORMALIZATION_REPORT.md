# Placeholder Normalization Report — Trainee Registration (v8 draft)

Generated 2026-08-25 · manifest contract `1.0` · mapping source `CURRENT_MERGE_TEMPLATE_FIELDS`

> **Outcome: zero token edits were required.** All 49 placeholders in the template already match the manifest byte-for-byte. The literal `<<Ho_ten>>` / `<<Ngay_ky_*>>` text the operator sees in Preview is **not** a template defect — it is the unresolved-placeholder fallback rendering when mapping rows are absent at render time. Evidence in §D.

---

## A. PLACEHOLDER AUDIT SUMMARY

```
TOTAL_PLACEHOLDERS=49        (unique keys; 76 occurrences)
EXACT_MATCH=49
ALIASES_NORMALIZED=0
CASE_MISMATCH=0
UNKNOWN=0
MISSING_REQUIRED=0
```

Verified three independent ways: (1) token extraction via the production `extractUniquePlaceholders()`; (2) full `diff` of the template body against the v7 and v8 migration bodies — placeholder sets identical in all three; (3) live `buildTemplateManifest()` run → `placeholderCount 49`, `unmapped: []`.

### Placeholder inventory

| # | Current | Status | Canonical | Action | Uses | Mapping (type → source) | Reason |
|---|---|---|---|---|---|---|---|
| 1 | `{{Code}}` | EXACT_MATCH | `Code` | none | 2 | `CORE_FIELD` → `code` | Key exists in manifest verbatim; resolves at runtime |
| 2 | `{{Cong_ty_thu_nhap_khac}}` | EXACT_MATCH | `Cong_ty_thu_nhap_khac` | none | 1 | `CORE_FIELD` → `customAnswers.cong_ty_thu_nhap_khac` | Key exists in manifest verbatim; resolves at runtime |
| 3 | `{{Cong_viec_hien_tai_Khac}}` | EXACT_MATCH | `Cong_viec_hien_tai_Khac` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.cong_viec_hien_tai == "Khác"` | Key exists in manifest verbatim; resolves at runtime |
| 4 | `{{Cong_viec_hien_tai_Sinh_vien}}` | EXACT_MATCH | `Cong_viec_hien_tai_Sinh_vien` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.cong_viec_hien_tai == "Sinh viên"` | Key exists in manifest verbatim; resolves at runtime |
| 5 | `{{Cong_viec_hien_tai_khac}}` | EXACT_MATCH | `Cong_viec_hien_tai_khac` | none | 1 | `CORE_FIELD` → `customAnswers.cong_viec_hien_tai_khac` | Key exists in manifest verbatim; resolves at runtime |
| 6 | `{{Cong_viec_khac}}` | EXACT_MATCH | `Cong_viec_khac` | none | 1 | `CORE_FIELD` → `customAnswers.cong_viec_khac` | Key exists in manifest verbatim; resolves at runtime |
| 7 | `{{Da_tung_lam_DHF_Co}}` | EXACT_MATCH | `Da_tung_lam_DHF_Co` | none | 1 | `CHECKBOX_OPTION` → `declaredType == "OLD"` | Key exists in manifest verbatim; resolves at runtime |
| 8 | `{{Da_tung_lam_DHF_Khong}}` | EXACT_MATCH | `Da_tung_lam_DHF_Khong` | none | 1 | `CHECKBOX_OPTION` → `declaredType == "NEW"` | Key exists in manifest verbatim; resolves at runtime |
| 9 | `{{Dia_chi_tam_tru}}` | EXACT_MATCH | `Dia_chi_tam_tru` | none | 2 | `CORE_FIELD` → `residentialAddress` | Key exists in manifest verbatim; resolves at runtime |
| 10 | `{{Dia_chi_thuong_tru}}` | EXACT_MATCH | `Dia_chi_thuong_tru` | none | 2 | `CORE_FIELD` → `permanentAddress` | Key exists in manifest verbatim; resolves at runtime |
| 11 | `{{Dia_diem_ky}}` | EXACT_MATCH | `Dia_diem_ky` | none | 2 | `CORE_FIELD` → `location` | Key exists in manifest verbatim; resolves at runtime |
| 12 | `{{Dia_diem_thu_nhap_khac}}` | EXACT_MATCH | `Dia_diem_thu_nhap_khac` | none | 1 | `CORE_FIELD` → `customAnswers.dia_diem_thu_nhap_khac` | Key exists in manifest verbatim; resolves at runtime |
| 13 | `{{Email}}` | EXACT_MATCH | `Email` | none | 1 | `CORE_FIELD` → `customAnswers.email` | Key exists in manifest verbatim; resolves at runtime |
| 14 | `{{Ho_ten}}` | EXACT_MATCH | `Ho_ten` | none | 10 | `CORE_FIELD` → `fullName` | Key exists in manifest verbatim; resolves at runtime |
| 15 | `{{Khu_vuc_Da_Lat}}` | EXACT_MATCH | `Khu_vuc_Da_Lat` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.khu_vuc_lam_viec_truoc_day == "Đà Lạt"` | Key exists in manifest verbatim; resolves at runtime |
| 16 | `{{Khu_vuc_Da_Quy}}` | EXACT_MATCH | `Khu_vuc_Da_Quy` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.khu_vuc_lam_viec_truoc_day == "Đa Quý"` | Key exists in manifest verbatim; resolves at runtime |
| 17 | `{{Khu_vuc_Da_Ron}}` | EXACT_MATCH | `Khu_vuc_Da_Ron` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.khu_vuc_lam_viec_truoc_day == "Đạ Ròn"` | Key exists in manifest verbatim; resolves at runtime |
| 18 | `{{Khu_vuc_Khac}}` | EXACT_MATCH | `Khu_vuc_Khac` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.khu_vuc_lam_viec_truoc_day == "Khác"` | Key exists in manifest verbatim; resolves at runtime |
| 19 | `{{Khu_vuc_Lam_Ha}}` | EXACT_MATCH | `Khu_vuc_Lam_Ha` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.khu_vuc_lam_viec_truoc_day == "Lâm Hà"` | Key exists in manifest verbatim; resolves at runtime |
| 20 | `{{Loai_cong_viec_Cong_nhan}}` | EXACT_MATCH | `Loai_cong_viec_Cong_nhan` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.loai_cong_viec_truoc_day == "Công nhân"` | Key exists in manifest verbatim; resolves at runtime |
| 21 | `{{Loai_cong_viec_Lao_dong_tap_nghe}}` | EXACT_MATCH | `Loai_cong_viec_Lao_dong_tap_nghe` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.loai_cong_viec_truoc_day == "Lao động tập nghề"` | Key exists in manifest verbatim; resolves at runtime |
| 22 | `{{Loai_cong_viec_Nhan_vien}}` | EXACT_MATCH | `Loai_cong_viec_Nhan_vien` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.loai_cong_viec_truoc_day == "Nhân viên"` | Key exists in manifest verbatim; resolves at runtime |
| 23 | `{{Nam_thue}}` | EXACT_MATCH | `Nam_thue` | none | 2 | `COMPUTED_FIELD` → `DATE_YEAR` | Key exists in manifest verbatim; resolves at runtime |
| 24 | `{{Ngay_cap_CCCD}}` | EXACT_MATCH | `Ngay_cap_CCCD` | none | 2 | `CORE_FIELD` → `dateOfIssue` | Key exists in manifest verbatim; resolves at runtime |
| 25 | `{{Ngay_ky_day}}` | EXACT_MATCH | `Ngay_ky_day` | none | 3 | `COMPUTED_FIELD` → `DATE_DAY` | Key exists in manifest verbatim; resolves at runtime |
| 26 | `{{Ngay_ky_month}}` | EXACT_MATCH | `Ngay_ky_month` | none | 3 | `COMPUTED_FIELD` → `DATE_MONTH` | Key exists in manifest verbatim; resolves at runtime |
| 27 | `{{Ngay_ky_year}}` | EXACT_MATCH | `Ngay_ky_year` | none | 3 | `COMPUTED_FIELD` → `DATE_YEAR` | Key exists in manifest verbatim; resolves at runtime |
| 28 | `{{Ngay_nhan_viec}}` | EXACT_MATCH | `Ngay_nhan_viec` | none | 2 | `CORE_FIELD` → `startingDate` | Key exists in manifest verbatim; resolves at runtime |
| 29 | `{{Ngay_sinh}}` | EXACT_MATCH | `Ngay_sinh` | none | 3 | `CORE_FIELD` → `dob` | Key exists in manifest verbatim; resolves at runtime |
| 30 | `{{Ngay_tiep_nhan}}` | EXACT_MATCH | `Ngay_tiep_nhan` | none | 1 | `CORE_FIELD` → `startingDate` | Key exists in manifest verbatim; resolves at runtime |
| 31 | `{{Nguoi_tiep_nhan}}` | EXACT_MATCH | `Nguoi_tiep_nhan` | none | 1 | `SYSTEM_FIELD` → `CURRENT_USER_NAME` | Key exists in manifest verbatim; resolves at runtime |
| 32 | `{{Noi_cap_CCCD}}` | EXACT_MATCH | `Noi_cap_CCCD` | none | 1 | `CORE_FIELD` → `placeOfIssue` | Key exists in manifest verbatim; resolves at runtime |
| 33 | `{{So_CCCD}}` | EXACT_MATCH | `So_CCCD` | none | 3 | `CORE_FIELD` → `cccd` | Key exists in manifest verbatim; resolves at runtime |
| 34 | `{{So_dien_thoai}}` | EXACT_MATCH | `So_dien_thoai` | none | 2 | `CORE_FIELD` → `phone` | Key exists in manifest verbatim; resolves at runtime |
| 35 | `{{So_dinh_danh_cu}}` | EXACT_MATCH | `So_dinh_danh_cu` | none | 1 | `CORE_FIELD` → `customAnswers.so_dinh_danh_cu` | Key exists in manifest verbatim; resolves at runtime |
| 36 | `{{So_tai_khoan}}` | EXACT_MATCH | `So_tai_khoan` | none | 1 | `CORE_FIELD` → `customAnswers.so_tai_khoan` | Key exists in manifest verbatim; resolves at runtime |
| 37 | `{{TKNH_Chua_co}}` | EXACT_MATCH | `TKNH_Chua_co` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.tinh_trang_tknh == "Chưa có"` | Key exists in manifest verbatim; resolves at runtime |
| 38 | `{{TKNH_Da_co}}` | EXACT_MATCH | `TKNH_Da_co` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.tinh_trang_tknh == "Đã có"` | Key exists in manifest verbatim; resolves at runtime |
| 39 | `{{Tap_nghe_Ban_hang}}` | EXACT_MATCH | `Tap_nghe_Ban_hang` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.tap_nghe_nguyen_vong == "Bán hàng"` | Key exists in manifest verbatim; resolves at runtime |
| 40 | `{{Tap_nghe_Dong_goi}}` | EXACT_MATCH | `Tap_nghe_Dong_goi` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.tap_nghe_nguyen_vong == "Đóng gói"` | Key exists in manifest verbatim; resolves at runtime |
| 41 | `{{Tap_nghe_Khac}}` | EXACT_MATCH | `Tap_nghe_Khac` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.tap_nghe_nguyen_vong == "Khác"` | Key exists in manifest verbatim; resolves at runtime |
| 42 | `{{Tap_nghe_Trong_cham_soc_thu_hoach}}` | EXACT_MATCH | `Tap_nghe_Trong_cham_soc_thu_hoach` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.tap_nghe_nguyen_vong == "Trồng, chăm sóc, thu hoạch"` | Key exists in manifest verbatim; resolves at runtime |
| 43 | `{{Ten_ngan_hang}}` | EXACT_MATCH | `Ten_ngan_hang` | none | 1 | `CORE_FIELD` → `customAnswers.ten_ngan_hang` | Key exists in manifest verbatim; resolves at runtime |
| 44 | `{{Ten_truong}}` | EXACT_MATCH | `Ten_truong` | none | 1 | `CORE_FIELD` → `customAnswers.ten_truong` | Key exists in manifest verbatim; resolves at runtime |
| 45 | `{{Thu_nhap_Chi_DHF}}` | EXACT_MATCH | `Thu_nhap_Chi_DHF` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.nguon_thu_nhap == "Chỉ phát sinh tại Dalat Hasfarm"` | Key exists in manifest verbatim; resolves at runtime |
| 46 | `{{Thu_nhap_Ngoai_DHF}}` | EXACT_MATCH | `Thu_nhap_Ngoai_DHF` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.nguon_thu_nhap == "Phát sinh ngoài Dalat Hasfarm"` | Key exists in manifest verbatim; resolves at runtime |
| 47 | `{{Tien_an_tien_su_Co}}` | EXACT_MATCH | `Tien_an_tien_su_Co` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.tien_an_tien_su == "Có"` | Key exists in manifest verbatim; resolves at runtime |
| 48 | `{{Tien_an_tien_su_Khong}}` | EXACT_MATCH | `Tien_an_tien_su_Khong` | none | 1 | `CHECKBOX_OPTION` → `customAnswers.tien_an_tien_su == "Không"` | Key exists in manifest verbatim; resolves at runtime |
| 49 | `{{dia_chi_cu_tru}}` | EXACT_MATCH | `dia_chi_cu_tru` | none | 1 | `CORE_FIELD` → `residentialAddress` | Key exists in manifest verbatim; resolves at runtime |

### MISSING_REQUIRED_PLACEHOLDERS

```
MISSING_REQUIRED_PLACEHOLDERS=none
```

The manifest marks 6 placeholders `isRequired: true` — `Ho_ten`, `Ngay_sinh`, `Dia_chi_tam_tru`, `So_dien_thoai`, `So_CCCD`, `Ngay_nhan_viec`. All 6 are present in the HTML. Nothing was auto-inserted.

### Duplicate report

| PLACEHOLDER | COUNT | VALID_DUPLICATE | Notes |
|---|---|---|---|
| `Ho_ten` | 10 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Ngay_ky_day` | 3 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Ngay_ky_month` | 3 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Ngay_ky_year` | 3 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Ngay_sinh` | 3 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `So_CCCD` | 3 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Code` | 2 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Dia_chi_tam_tru` | 2 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Dia_chi_thuong_tru` | 2 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Dia_diem_ky` | 2 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Nam_thue` | 2 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Ngay_cap_CCCD` | 2 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `Ngay_nhan_viec` | 2 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |
| `So_dien_thoai` | 2 | yes | Repeated once per logical sub-document (6 sub-forms, each with its own header/signature block) |

No duplicates were removed. All repeats are structurally legitimate: the document is 6 logical sub-forms (`logicalPageCount: 6`), each carrying its own identity and signature block, so `Ho_ten` recurring 10× is expected.

---

## B. PROBLEM PLACEHOLDERS

The 8 keys reported as rendering literally. **Every one is present and correctly mapped** — none is a spelling, alias, or case problem.

```
PLACEHOLDER=Ho_ten
FOUND_IN_MANIFEST=yes
CANONICAL=Ho_ten
SOURCE_PATH=fullName
MAPPING_TYPE=CORE_FIELD
ACTION=none

PLACEHOLDER=Dia_diem_ky
FOUND_IN_MANIFEST=yes
CANONICAL=Dia_diem_ky
...
FOUND_IN_MANIFEST=yes
CANONICAL=Ngay_ky_day
SOURCE_PATH=(computed: DATE_DAY)
MAPPING_TYPE=COMPUTED_FIELD
ACTION=none

PLACEHOLDER=Ngay_ky_month
FOUND_IN_MANIFEST=yes
CANONICAL=Ngay_ky_month
SOURCE_PATH=(computed: DATE_MONTH)
MAPPING_TYPE=COMPUTED_FIELD
ACTION=none

PLACEHOLDER=Ngay_ky_year
FOUND_IN_MANIFEST=yes
CANONICAL=Ngay_ky_year
SOURCE_PATH=(computed: DATE_YEAR)
MAPPING_TYPE=COMPUTED_FIELD
ACTION=none

PLACEHOLDER=dia_chi_cu_tru
FOUND_IN_MANIFEST=yes
CANONICAL=dia_chi_cu_tru
SOURCE_PATH=residentialAddress
MAPPING_TYPE=CORE_FIELD
ACTION=none

PLACEHOLDER=Dia_chi_thuong_tru
FOUND_IN_MANIFEST=yes
CANONICAL=Dia_chi_thuong_tru
SOURCE_PATH=permanentAddress
MAPPING_TYPE=CORE_FIELD
ACTION=none

PLACEHOLDER=Dia_chi_tam_tru
FOUND_IN_MANIFEST=yes
CANONICAL=Dia_chi_tam_tru
SOURCE_PATH=residentialAddress
MAPPING_TYPE=CORE_FIELD
ACTION=none
```

Note on `dia_chi_cu_tru`: the lowercase leading `d` looks like a case defect but is **the canonical manifest spelling**. Per the standing instruction not to guess capitalization, it is left exactly as-is.

The three `Ngay_ky_*` keys are `COMPUTED_FIELD`s with an empty `sourcePath`, so they are **not** `UNRESOLVED_SYSTEM_PLACEHOLDER` — the resolver derives them (see §D warning 4 for the date-source caveat). They were not swapped for any core field.
...
No token replacement was justified: `EXACT_MATCH=49`, `ALIASES_NORMALIZED=0`, `CASE_MISMATCH=0`, `UNKNOWN=0`. Editing any token would have moved a working key **off** its manifest entry. The HTML and CSS in §E/§F are therefore byte-identical to the current template — SHA-256 `d68b3296…f794e` (HTML) and `55e91fdc…2c757` (CSS), matching the v8 migration header.

---

## D. WARNINGS

**1 — Root cause: mapping rows are missing at render time, not a template defect.** Reproduced by rendering the real pipeline (`normalizeFullHtmlDocument` → `selectPreviewMappings` → `buildCanonicalSnapshot` → `renderCanonicalDocument`) under three conditions:

| Scenario | Mapping rows | Result |
|---|---|---|
| A. DRAFT v8 + 49 live `merge_template_fields` | 49 | `valid: true`, `unreplaced: []` — renders `Đà Lạt, ngày 02 tháng 03 năm 2026` |
| B. DRAFT v8 + **zero** rows | 0 | `valid: false`, **35 unreplaced** — renders `Đà Lạt, ngày {{Ngay_ky_day}} tháng {{Ngay_ky_month}} năm {{Ngay_ky_year}}` |
| C. PUBLISHED v8 + empty frozen snapshot | falls back to current 49 | `valid: true`, zero literals |

Scenario B reproduces the operator's exact symptom with an unmodified template. The partial nature of the failure is also explained: keys covered by `FALLBACK_PLACEHOLDER_MAP` (e.g. `Ho_ten` → `fullName`) still resolve without mapping rows, so the operator sees *some* fields fill and others stay literal. Both the v7 and v8 migrations insert the version with `status='DRAFT'` and `mapping_snapshot='[]'` and never publish — consistent with rows being absent or orphaned. **Fix belongs in data/mapping state, not in this HTML.** Operator guidance is already emitted by `buildUnresolvedPlaceholderWarning()`:

> Không thể xem trước đầy đủ: còn N trường chưa được thay thế: … Các trường này chưa có mapping — mở Mapping Inspector để gán nguồn dữ liệu trước khi Áp dụng/Xuất bản.

**2 — The `<<…>>` angle brackets in the bug report come from the warning banner, not the template.** The template body contains zero `<<` tokens (it is 100% `{{…}}`); `buildUnresolvedPlaceholderWarning()` formats unresolved keys as `<<key>>` for display. Confirmed at runtime that `<<X>>` and `{{X}}` resolve to the same key via `PLACEHOLDER_PATTERN`, so **no delimiter conversion is needed or was performed**. Chasing `<<Ho_ten>>` as a literal string in the HTML is a dead end.

**3 — Manifest case collision — do NOT fold.** `Cong_viec_hien_tai_Khac` and `Cong_viec_hien_tai_khac` differ only in one letter's case and are **two legitimately distinct fields**: the former is a `CHECKBOX_OPTION` (`customAnswers.cong_viec_hien_tai == "Khác"`), the latter a `CORE_FIELD` free-text answer. Verified rendering side by side → `A=☒ B=Thợ may tự do`. Any case-insensitive normalization pass **will corrupt this document**.

**4 — Signing-date placeholders silently derive from the wrong date.** `Ngay_ky_day`/`_month`/`_year` and `Nam_thue` are `COMPUTED_FIELD`s with an empty `sourcePath`; `data-resolver.ts:175` therefore defaults them to `startingDate`, **not** a distinct signing date. Documents will print the work-start date in the signature line. Out of scope for placeholder normalization — flagged for operator review.

**5 — Address placeholders must never be cross-substituted.** `Dia_chi_thuong_tru` → `permanentAddress`, while `dia_chi_cu_tru` and `Dia_chi_tam_tru` both → `residentialAddress`. The two-to-one mapping is intentional (`crossAddressFallback: false`); do not "deduplicate" it.

**6 — `incoming/test2.html` is disqualified as a runtime template.** It carries preview chrome (`<style id="uicss">`, toolbar) and inline JavaScript with demo data. It was not used as the normalization base. Normalization target was `templates/document-merge/trainee-registration/canonical-source.html`.

**7 — Safety scan clean.** Zero matches for `<script>`, `onclick`, `onload`, `onerror`, `javascript:`, or external stylesheets in both outputs, and zero entity-encoded (`&lt;&lt;`) placeholder tokens — so no entity decoding was applied. Nothing was removed.

---

## E. NORMALIZED HTML

Complete document, unchanged from source (SHA-256 `d68b3296…f794e`):
...
<div class="paper">
  ...
  <div class="body-narrow">
    ...
    <div class="line">Họ và Tên: {{Ho_ten}}           Sinh ngày: {{Ngay_sinh}}</div>
...
<div class="paper">
  ...
  <div class="line b">Người tiếp nhận: {{Nguoi_tiep_nhan}}     Ngày: {{Ngay_tiep_nhan}}</div>
...
<!-- Explicit v8 A4 boundary: regulations must start on a new physical page. -->
<div class="paper regulations-page">
  <div class="doc-header regulations-header">
    <div class="left">DALAT HASFARM</div>
...
<div class="paper">
  ...
  <div class="sign-block regulations-signature mt-8">
...
Complete stylesheet, unchanged from source (SHA-256 `55e91fdc…2c757`):
...
/* v8 pagination: scoped, deterministic boundary for the regulations section. */
@media print {
  .regulations-page { break-before: page; page-break-before: always; }
  .regulations-signature { break-inside: avoid; page-break-inside: avoid; }
}
```
...
```
PLACEHOLDER_NORMALIZATION_SAFE=yes
REQUIRES_OPERATOR_REVIEW=yes
```

`REQUIRES_OPERATOR_REVIEW=yes` because the actual defect is outside this template: restore the 49 `merge_template_fields` rows for the active version (§D.1), then re-check the signing-date source (§D.4).
